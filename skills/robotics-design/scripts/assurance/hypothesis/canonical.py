...
def canonical_value(value: object, path: str = "value") -> Any:
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in value.items():
            if not isinstance(key, str):
                raise ValueError(f"{path} keys must be strings for canonical JSON")
            copied[key] = canonical_value(item, f"{path}[{key}]")
...
def candidate_id(
    ...
    record = {
        ...
        "parent_id": validate_optional_identifier(parent_id, "parent_id"),
