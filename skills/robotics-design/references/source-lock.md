...
The installer removes Claude-only frontmatter fields from `ros2-engineering-skills` while retaining its scripts for manual Codex validation. Update commits only after source audit, structural validation, real artifact smoke tests, ROS script tests, routing regression tests, and visual-fidelity RED/GREEN regression tests when visualization behavior changes.
